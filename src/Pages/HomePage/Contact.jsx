...
import { useCreateInquiryMutation } from "../../redux/features/inquiryApi";
...
const Contact = () => {
  ...
  const [createInquiry, { isLoading, isSuccess, isError, error }] =
    useCreateInquiryMutation();

  const onSubmit = async (formData) => {
    try {
      const response = await createInquiry(formData).unwrap();
      showSuccess(response.message || "Enquiry submitted successfully");
      reset();
    } catch (err) {
      showError(err.data?.message || "Failed to submit enquiry");
    }
  };

  // Optional: clear any success/error messages after a timeout
  useEffect(() => {
